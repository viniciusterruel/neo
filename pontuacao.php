...
//Operações realizadas para a obtenção do vetor de vetores das primeiras mensagens de cada ticket---------------------------------
	...
	foreach($novo_json as $pedaco){
		if($valor_msg[$i] >= 4){
			...
			array_push($pedaco, $alta['Prioridade']);//adiciona um novo campo com a prioridade do ticket
			array_push($novo_json, $pedaco);//adiciona o novo ticket ao json
		}
		elseif($valor_msg[$i] < 4){
			array_push($pedaco, $valor_msg[$i]);//adiciona um novo campo ao json em cada ticket
			array_push($pedaco, $normal['Prioridade']);//adiciona um novo campo com a prioridade do ticket
			array_push($novo_json, $pedaco);//adiciona o novo ticket ao json
		}
	...
	session_start("envia_json");//começa sessão para enviar o json
	$_SESSION['dados'] = $novo_json;//envia o json para a página apineo.php
?>

<!--Formulário para envio do tipo de ordenação e filtros-->
...
<form class="register" action="apineo.php" method="post">
  <select class="basic simple" name="ordenar">
   ...
   <input type="radio" name="prioridade" value="Todos" checked="checked">Todos
   </p>
